...
for vendor in pagesrc1.find_all('div', class_=re.compile("^merchantLogo")):
...
for partNumber in pagesrc1.find_all('div', class_=re.compile("^col-xs-3 col-md-2 listProdImg")):
...
list1 = []
# selects the elements that have the product name information contained in the div class title
for title in pagesrc1.find_all('div', class_=re.compile("^title")):
...
for price in pagesrc1.find_all('div', id=re.compile("^offerPrice")):
...
for description in pagesrc1.find_all('p', class_=re.compile("^desc")):
